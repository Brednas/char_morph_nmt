...
from dgm4nlp.nlpmorphutils import MorphText
import re
...
class LemTokenizer:
    ...
    def fit_many(self, input_streams):
        ...
        for stream in input_streams:
            for line in stream:
                lems = [re.split(r'(\[[^\[\]]+\])', word)[0] for word in line.split()]
                self._counts.update(lems)
    ...
    def to_sequences(self, input_stream, dtype='int64'):
        ...
        return [np.array([self._vocab.get(lem, unk_id) for lem in
                          [re.split(r'(\[[^\[\]]+\])', word)[0] for word in self._tokenize(line)]], dtype=dtype)
                         for line in input_stream]

    def to_sequences_iterator(self, input_stream, dtype='int64'):
        unk_id = self._vocab[self._unk_str]
        for line in input_stream:
            yield np.array([self._vocab.get(lem, unk_id) for lem in
                            [re.split(r'(\[[^\[\]]+\])', word)[0] for word in self._tokenize(line)]], dtype=dtype)
...
class LemText:
    ...
    def __init__(self, input_path, tokenizer: LemTokenizer,
                 shortest=1,
                 longest=np.inf,
                 trim=False,
                 output_dir=None,
...
class Multitext:
    ...
    def __init__(self, input_paths: tuple, tokenizers: tuple,
        ...
        for i, TextClass in enumerate([CharText, Text, MorphText, LemText]):
