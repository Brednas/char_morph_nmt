...
from dgm4nlp.nlputils import Tokenizer # for use in bitext function
from dgm4nlp.nlputils import Text # for have Multitext refer to the word-based Text class
from dgm4nlp.nlpcharutils import CharTokenizer
from dgm4nlp.nlpcharutils import CharText
import re
...
class MorphTokenizer:
    ...
    A Tokenizer splits streams of text into tokens (on white spaces) and builds and internal vocabulary.
     The vocabulary can be bounded in size and it contains some standard symbols:
        ...
        To bound a vocabulary set nb_chars to a positive integer. This will cap the number of words in the vocabulary,
    ...
    def __init__(self, nb_words=None, bos_str=None, eos_str=None, eow_str = '[eow]', unk_str='[Unknown]', pad_str='-PAD-'):
        """
        ...
        self._eow_str = eow_str
        self._longest_xword = 0
        ...
        if eow_str is not None:
            self._vocab[eow_str] = len(self._tokens)
            self._tokens.append(eow_str)
    ...
    def fit_many(self, input_streams):
        ...
        for stream in input_streams:
            for line in stream:
                for word in line.split():
                    # This regular expression returns all bracketed morphemes (e.g. [Adj]
                    # It searches for groups of square brackets containing anything but square brackets
                    morphs = re.findall(r'(\[[^\[\]]+\])', word)+['[eow]']
                    self._counts.update(morphs)
                    if len(morphs) > self._longest_xword:
                        self._longest_xword = len(morphs) # to compensate for [eow]
        ...
        if self._nb_words:
            if len(self._vocab) > self._nb_words:
                print("Warning, limit to alphabet detected. Max morphs = % s, morphs types in corpus = % s" % (self._nb_words, len(self._vocab)))

    def vocab_size(self):
        return len(self._tokens)

    def longest_xword(self):
        return self._longest_xword

    def to_sequences(self, input_stream, dtype='object'):
        # does the same as to_sequences, but changes the stream to an integer per morpheme instead of
        # an integer per word
        unk_id = self._vocab[self._unk_str]
        return [np.array([[self._vocab.get(morph, unk_id) for morph in re.findall(r'(\[[^\[\]]+\])', word)+[self._eow_str]]
                          for word in self._tokenize(line)], dtype=dtype)
                for line in input_stream]

    def to_sequences_iterator(self, input_stream, dtype='object'):
        unk_id = self._vocab[self._unk_str]
        for i, line in enumerate(input_stream):
            yield np.array([[self._vocab.get(morph, unk_id) for morph in re.findall(r'(\[[^\[\]]+\])', word)+[self._eow_str]]
                            for word in self._tokenize(line)], dtype=dtype)

    # This function is not yet implemented for morphemes
    # Currently transforming the input data to integeres removes information such as surface from
    # and inflectional/derivational. Thus exactly reversing the process is currently not possible.
    ...
    def write_as_char_text(self, matrix, output_stream, skip_pad=True):
        if skip_pad:
            for seq in matrix:
                print(' '.join([''.join([self._tokens[character] for character in word if character != 0])
                                for word in itertools.takewhile(lambda x: x[0] != 0, seq)]), file=output_stream)
        else:
            for seq in matrix:
                print(' '.join(''.join([self._tokens[cid] for cid in word]) for word in seq), file=output_stream)
...
def construct_mmap(input_path, output_path, tokenizer, selection, nb_tokens, dtype):
    ...
    """
    longest_word = tokenizer.longest_xword()

    # construct memory mapped array
    mmap = np.memmap(output_path, dtype=dtype, mode='w+', shape=(nb_tokens,longest_word))
    ...
    for sid, seq in enumerate(tokenizer.to_sequences_iterator(smart_ropen(input_path))):
        if not selection[sid]:  # skip sentences that do not comply with length constraints
            continue
        for wid, word in enumerate(seq):
            # here we have a valid sequence, thus we memory map it
            mmap[offset + wid][:len(word)] = word
        offset += seq.shape[0]
...
class MorphText:
    ...
    def __init__(self, input_path, tokenizer: MorphTokenizer,
                 shortest=1,
                 longest=np.inf,
                 trim=False,
                 output_dir=None,
    ...
    def batch_iterator(self, batch_size, endless=False, shorter_batch='mask', dynamic_sequence_length=False, ):
        ...
        longest_xword = self._tokenizer.longest_xword()
        mmap = np.memmap(self._memmap_path, dtype=self._batch_dtype, mode='r', shape=(nb_tokens, longest_xword))
        ...
        generating = True
        while generating:
            batch = np.zeros((batch_size, n_cols, longest_xword), dtype=self._batch_dtype)
            mask = np.zeros((batch_size, n_cols, longest_xword), dtype=self._mask_dtype)
            valid_batch = True
            longest_in_batch = 0
            samples_in_batch = 0
...
class Multitext:
    ...
    def __init__(self, input_paths: tuple, tokenizers: tuple,
        ...
        for i, TextClass in enumerate([CharText,Text,MorphText]):
                corpora.append(TextClass(input_path=input_paths[i],
