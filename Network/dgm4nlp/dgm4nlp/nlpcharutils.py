...
class CharTokenizer:
    ...
    def fit_many(self, input_streams):
        ...
        for stream in input_streams:
            for line in stream:
                for word in line.split():
                    self._counts.update(list(word))
                    if len(word) > self._longest_xword:
                        self._longest_xword = len(word)
        ...
        if self._nb_words:
            if len(self._vocab) > self._nb_words:
                print("Warning, limit to alphabet detected. Max characters = % s, character types in corpus = % s" % (self._nb_words, len(self._vocab)))
    ...
    def to_sequences(self, input_stream, dtype='object'):
        # does the same as to_sequences, but changes the stream to an integer per character instead of
        # an integer per word
        unk_id = self._vocab[self._unk_str]
        return [np.array([[self._vocab.get(char, unk_id) for char in word]
    ...
    def to_sequences_iterator(self, input_stream, dtype='object'):
        unk_id = self._vocab[self._unk_str]
        for line in input_stream:
            yield np.array([[self._vocab.get(char, unk_id) for char in word]
                            for word in self._tokenize(line)], dtype=dtype)
    ...
    def write_as_char_text(self, matrix, output_stream, skip_pad=True):
        if skip_pad:
            for seq in matrix:
                print(' '.join([''.join([self._tokens[character] for character in word if character != 0]) for word in itertools.takewhile(lambda x: x[0] != 0, seq)]), file=output_stream)
        else:
            for seq in matrix:
...
class CharText:
    ...
    def __init__(self, input_path, tokenizer: CharTokenizer,
                 shortest=1,
                 longest=np.inf,
                 trim=False,
                 output_dir=None,
...
class Multitext:
    ...
    def __init__(self, input_paths: tuple, tokenizers: tuple,
        ...
        # One textclass should be provided per stream, in the order that they appear.
        # This list could be made an input variable
        for i, TextClass in enumerate([CharText,Text]):
