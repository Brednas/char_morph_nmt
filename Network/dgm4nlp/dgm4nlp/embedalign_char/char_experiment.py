...
from dgm4nlp.nlpcharutils import Multitext
...
class ExperimentWrapper:
    ...
    def __init__(self, training_paths: list,
                 ...
                 # data pre-processing
                 nb_words: list, # first item contains max number of characters (suggested assigned none), second contains max words in y vocab
        ...
        """
        assert len(training_paths) == 2, 'I expect a bitext'

        # prepare data
        nb_streams = len(training_paths)
        logging.info('Fitting vocabularies')
        tks = []
        for i, (path, vs, bos) in enumerate(zip(training_paths, nb_words, bos_str)):
            if i == 0: # The source language (first path) will be handled character level
                logging.info(' stream=%d', i)
                # tokenizer with a bounded vocabulary
                tks.append(CharTokenizer(nb_words=vs, bos_str=bos))
                tks[-1].fit_one(smart_ropen(path))
                logging.info('  vocab-size=%d', tks[-1].vocab_size())
            else: # Other languages in the stream are handled at word level
                logging.info(' stream=%d', i)
                # tokenizer with a bounded vocabulary
                tks.append(Tokenizer(nb_words=vs, bos_str=bos))
                tks[-1].fit_one(smart_ropen(path))
                logging.info('  vocab-size=%d', tks[-1].vocab_size())
        ...
        if validation_paths:
            ...
            validation = Multitext(validation_paths[0:2],
                                   tokenizers=tks,
                                   shortest=shortest_sequence,
                                   longest=longest_sequence,
                                   trim=[dynamic_sequence_length] * nb_streams,
            ...
            if len(validation_paths) == 3:  # we have a NAACL file for alignments
                logging.info("Working with gold labels for validation: '%s'", validation_paths[2])
                # reads in sets of gold alignments
                val_gold_alignments = read_naacl_alignments(validation_paths[2])
        ...
        if test_paths:
            ...
            if len(test_paths) == 3:  # we have a NAACL file for alignments
                logging.info("Working with gold labels for test: '%s'", test_paths[2])
                # reads in sets of gold alignments
                test_gold_alignments = read_naacl_alignments(test_paths[2])
                # discard those associated with sentences that are no longer part of the validation set
                #  (for example due to length constraints)
