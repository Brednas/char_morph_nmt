...
def make_morph_auto_encoder(x_vocab_size,
    ...
    conv_c2 = layers.convolutional.Conv1D(filters=150, kernel_size=2, strides=1, padding='valid',
    ...
    conv_c3 = layers.convolutional.Conv1D(filters=93, kernel_size=3, strides=1, padding='valid',
    ...
    conv_c4 = layers.convolutional.Conv1D(filters=13, kernel_size=4, strides=1, padding='valid',
    ...
    dim2_c = layers.Lambda(lambda t: remove_middle_dim(t), output_shape=(num_filters,), name='dim2_c')(conv_concat)

    x_embeddings = layers.Lambda(lambda pair: back_to_t3(pair[0], pair[1]), output_shape=(longest_xsnt, num_filters), name='word_emb_x')(
        [dim2_c, x])
    ...
    pm_x = layers.Activation('softmax',name = '{}.pm_x'.format(name))(xm_gru)
    #pm_x = layers.Lambda(lambda t: collapse_M_Mph(t),name = '{}.pm_x'.format(name), output_shape=())(pm_x_t4)
...
def test_morph_auto_encoder(training_paths,
    ...
    # this gets the model
    print(experiment.tks[0].vocab_size(),experiment.tks[1].vocab_size(),experiment.tks[1].vocab_size(),experiment.tks[2].vocab_size(),experiment.tks[2].longest_xword())
