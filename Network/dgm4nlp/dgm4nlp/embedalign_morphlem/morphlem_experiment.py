...
from dgm4nlp.morphlemutils import Multitext
...
class ExperimentWrapper:
    ...
    def __init__(self, training_paths: list,
        ...
        """
        assert len(training_paths) == 4, 'I expect a bitext and corresponding morphemes and lemmas'
        ...
        print(training_paths,tks,shortest_sequence,longest_sequence,bos_str,nb_streams)
        ...
        if validation_paths:
            ...
            validation = Multitext(validation_paths[0:4],
                                   tokenizers=tks,
                                   shortest=shortest_sequence,
                                   longest=longest_sequence,
                                   trim=[dynamic_sequence_length] * nb_streams,
            ...
            if len(validation_paths) == 5:  # we have a NAACL file for alignments
                logging.info("Working with gold labels for validation: '%s'", validation_paths[3])
                # reads in sets of gold alignments
                val_gold_alignments = read_naacl_alignments(validation_paths[4])
        ...
        if test_paths:
            ...
            if len(test_paths) == 5:  # we have a NAACL file for alignments
