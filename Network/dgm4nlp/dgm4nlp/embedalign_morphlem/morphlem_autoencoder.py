...
def make_morphlem_auto_encoder(x_vocab_size,
                            y_vocab_size,
                            x_char_vocab_size,
                            x_morph_vocab_size,
                            x_lem_vocab_size,
    ...
    #TODO makes the above a dense layer?
    pm_x = layers.Lambda(lambda t: collapse_M_Mph(t),name = '{}.pm_x'.format(name), output_shape=(None, x_morph_vocab_size))(pm_x_t4)

    # 3. Model P(L|X)
    x_lem = TimeDistributed(layers.Bidirectional(layers.recurrent.GRU(units=lem_emb_dim,
                                                return_sequences=False,
                                                dropout=rnn_dropout,
                                                recurrent_dropout=rnn_recurrent_dropout),
                                                merge_mode='sum'),
                                                name='lemma_rnn')(emb_c)

    pl_x = TimeDistributed(layers.Dense(x_lem_vocab_size, activation = 'softmax'), name='{}.pl_x'.format(name))(x_lem)
    ...
    # 4.b P(Y|X,A) = P(Y|X_A)
    # (B, M, Vy)
    x_sz = Concatenate(axis=-1, name='x_both_embeddings'.format(name))([x_embeddings, x_lem])

    py_xa = td.Softmax(y_vocab_size, hidden_layers=hidden_layers, name='{}.py_xa'.format(name))(x_sz)
    ...
    model = Model(inputs=[x, y, m], outputs=[py_x, pm_x, pl_x], name=name)
...
def test_morphlem_auto_encoder(training_paths,
                            validation_paths=[],
                            test_paths=[],
    ...
    # this gets the model
    model = make_morphlem_auto_encoder(x_vocab_size=experiment.tks[0].vocab_size(),
