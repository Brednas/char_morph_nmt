...
from dgm4nlp.embedalign_morphlem.morphlem_experiment import ExperimentWrapper
...
from dgm4nlp.morphlemutils import LemTokenizer
from dgm4nlp.nlputils import Tokenizer
from dgm4nlp.dim_utilities import remove_middle_dim4
from dgm4nlp.dim_utilities import remove_last, collapse_M_Mph
...
class BitextGenerator(Generator):
    ...
    def __init__(self, nb_classes, nb_morphs, nb_lems,
                 shorter_batch='trim',
                 endless=True,
        ...
        self.nb_lems = nb_lems

    def get(self, quadtext, batch_size):
        for (x, mx), (y, my), (morph_data, masked_morphs), (lem_data, masked_lems) in quadtext.batch_iterator(batch_size,
            ...
            # B, M, Vl
            l_labels = np.reshape(to_categorical(lem_data.flatten(), self.nb_lems),
                                  (lem_data.shape[0],lem_data.shape[1],self.nb_lems))

            yield [x, y, morph_data], {'AE.py_x' : y_labels, 'AE.pm_x': m_labels, 'AE.pl_x' : l_labels}
...
def viterbi_alignments(inputs, predictions):
    ...
    """
    batch_x, batch_y, batch_m = inputs
    batch_py_x, batch_pa_x, batch_py_xa, batch_pm_x, batch_pl_x = predictions
    B, N = batch_y.shape
...
def make_auto_encoder(x_vocab_size,
    ...
    cnn_embedding = layers.Lambda(lambda t: remove_middle_dim4(t), output_shape=(longest_xsnt,sum(num_filters)), name='cnn_embedding')(conv_concat)

    if nb_birnn_layers > 0:  # apply a number of BiRNN layers
        cnn_embedding = td.BiRNN(rnn_units,
                              dropout=rnn_dropout,
                              recurrent_dropout=rnn_recurrent_dropout,
                              merge_mode=rnn_merge_mode,
                              nb_layers=nb_birnn_layers,
                              architecture='gru',
                              name='{}.cnn-BiRNN'.format(name))(cnn_embedding)
    ...
    xrep = TimeDistributed(layers.core.RepeatVector(most_xmorphs))(cnn_embedding)
    ...
    xm_gru = TimeDistributed(layers.recurrent.GRU(units=x_morph_vocab_size,
                               ...
                               implementation=2,
                               input_dim=rnn_units + morph_emb_dim,
    ...
    # 3. Model P(L|X)
    rnn_embedding = TimeDistributed(layers.Bidirectional(layers.recurrent.GRU(units=lem_emb_dim,
                                                return_sequences=False,
                                                dropout=rnn_dropout,
                                                recurrent_dropout=rnn_recurrent_dropout),
                                                merge_mode=rnn_merge_mode),
                                                name='rnn_embedding')(emb_c)

    if nb_birnn_layers > 0:  # apply a number of BiRNN layers
        rnn_embedding = td.BiRNN(rnn_units,
                              dropout=rnn_dropout,
                              recurrent_dropout=rnn_recurrent_dropout,
                              merge_mode=rnn_merge_mode,
                              nb_layers=nb_birnn_layers,
                              architecture='gru',
                              name='{}.rnn-BiRNN'.format(name))(rnn_embedding)


    pl_x = TimeDistributed(layers.Dense(x_lem_vocab_size, activation = 'softmax'), name='{}.pl_x'.format(name))(rnn_embedding)

    # 4. Generative model P(Y|X=x)

    # 4.a Build an alignment model P(A|X,M,N)
    # (B, N, M)
    ...
    # 4.c Marginalise alignments: \sum_a P(a|x) P(Y|x,a)
    # (B, N, Vy)
...
def test_auto_encoder(training_paths,
                            validation_paths=[],
                            test_paths=[],
                            tok_classes = [CharTokenizer, Tokenizer, MorphTokenizer, LemTokenizer],
                            # data generation
                            vocab_size=[1000, 1000, 1000, 1000],
    ...
    assert len(training_paths) == 4, 'I expect 4 texts'
    ...
    experiment = ExperimentWrapper(training_paths=training_paths,
                                   ...
                                   bos_str=['-NULL-', None, '-NULL-','-NULL-'],  # for now conditional models always use a fake NULL input
                                   dynamic_sequence_length=dynamic_sequence_length)
    ...
    model = make_auto_encoder(x_vocab_size=experiment.tks[0].vocab_size(),
                                    ...
                                    x_lem_vocab_size = experiment.tks[3].vocab_size(),
                                    # encoders
                                    char_emb_dim=char_emb_dim,
                                    lem_emb_dim = lem_emb_dim,
    ...
    generator = BitextGenerator(experiment.tks[1].vocab_size(), # y's vocab
                                ...
                                experiment.tks[3].vocab_size(), # lemma vocab
                                dynamic_sequence_length=dynamic_sequence_length)
