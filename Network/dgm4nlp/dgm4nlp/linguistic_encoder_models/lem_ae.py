...
from dgm4nlp.embedalign_lem.lem_experiment import ExperimentWrapper
...
from dgm4nlp.lemutils import LemTokenizer
...
class BitextGenerator(Generator):
    ...
    def __init__(self, nb_classes, nb_lems,
                 shorter_batch='trim',
                 endless=True,
    ...
    def get(self, tritext, batch_size):
        for (x, mx), (y, my), (lem_data, masked_lems) in tritext.batch_iterator(batch_size,
            ...
            yield [x, y], {'AE.py_x' : y_labels, 'AE.pl_x' : l_labels}
...
def viterbi_alignments(inputs, predictions):
    ...
    batch_py_x, batch_pa_x, batch_py_xa, batch_pl_x = predictions
    B, N = batch_y.shape
...
def make_auto_encoder(x_vocab_size,
    ...
    model = Model(inputs=[x, y], outputs=[py_x, pa_x, py_xa, pl_x], name=name)
...
def test_auto_encoder(training_paths,
                            validation_paths=[],
                            test_paths=[],
                            tok_classes = [CharTokenizer, Tokenizer, LemTokenizer],
                            # data generation
    ...
    assert len(training_paths) == 3, 'I expect 3 texts'
    ...
    model = make_auto_encoder(x_vocab_size=experiment.tks[0].vocab_size(),
                                    ...
                                    x_lem_vocab_size = experiment.tks[2].vocab_size(),
                                    # encoders
    ...
    generator = BitextGenerator(experiment.tks[1].vocab_size(), # y's vocab
                                experiment.tks[2].vocab_size(), # lemma vocab
                                dynamic_sequence_length=dynamic_sequence_length)
