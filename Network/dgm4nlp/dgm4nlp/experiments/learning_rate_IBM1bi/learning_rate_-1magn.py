...
base_dir = '/mnt/data/sander/experiments/learning_rate_IBM1bi'
os.makedirs(base_dir, exist_ok=True)  # make sure base_dir exists
output_dir = tempfile.mkdtemp(prefix = 'lr_-1', suffix=datetime.now().strftime("%y-%m-%d.%Hh%Mm%Ss."), dir=base_dir)
...
test_auto_encoder(training_paths=['/mnt/data/sander/data/tr.norm',
                  ...
                  batch_size=200,
                  nb_epochs=100,
                  optimizer=optimizers.Adam(lr=0.0001), # 0.001 is the default suggested by the original paper and Keras
